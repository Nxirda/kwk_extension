...
TTS_CASE("Check for kwk::transform_reduce(In1, In2, Reduce, Transform, init) 1D")
{
  ...
  auto res = kwk::transform_reduce("simd", d1, d2, 10,
            std::pair{eve::add, 0},
...
TTS_CASE("Check for kwk::transform_reduce(In1, In2, Reduce, Transform, init) 2D")
{
  ...
  auto res = kwk::transform_reduce("simd", d1, d2, 0,
            std::pair{eve::add, 0},
            [](auto i1, auto i2){ return (i1*i2); }
            );
...
TTS_CASE("Check for kwk::inner_product(In1, In2, value, sum, product) 1D")
{
  ...
  auto res = kwk::inner_product("simd", d1, d2, 0,
            std::pair{eve::add, 0},
            [](auto i1, auto i2){ return (i1+i2); }
            );
...
TTS_CASE("Check for kwk::inner_product(In1, In2) 3D")
{
  ...
  auto std_res = std::inner_product(data1, &data1[2*3*4], data2, 0);

  TTS_EQUAL(res, std_res);
}; 
...
TTS_CASE("Check for kwk::inner_product(In1, In2) 4D")
{
  ...
  auto std_res = std::inner_product(data1, &data1[2*3*4*5], data2, 0);
...
TTS_CASE("Check for kwk::transform_inclusive_scan(In, Out, init, f1, f2) 1D")
{
  ...
  kwk::transform_inclusive_scan("simd", d1, d2, 0, 
          std::pair{eve::add, 0},
          [](auto i1){ return (i1); });
